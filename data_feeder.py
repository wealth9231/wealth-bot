...
def adaptive_parameters(adx, atr, price, market_state):
    ...
    if market_state == "趋势市":
        if adx > 40: stop_mult, tp_mult, position_pct, rsi_buy, rsi_sell, min_rr = 2.5, 3.5, TARGET_PROFIT_PCT*1.3, 75, 25, 1.1
        elif adx > 30: stop_mult, tp_mult, position_pct, rsi_buy, rsi_sell, min_rr = 2.0, 3.0, TARGET_PROFIT_PCT, 70, 30, 1.2
        else: stop_mult, tp_mult, position_pct, rsi_buy, rsi_sell, min_rr = 1.5, 2.5, TARGET_PROFIT_PCT*0.7, 65, 35, 1.3
...
def run_strategy():
    ...
    for c in coins:
        ...
        if size > 0:
            adaptive_info = f"止损{adaptive['stop_mult']}x"
            msg = format_signal_card(c, direction, market_state, price, adx, rsi, stop_loss, take_profit, size, strategy, adaptive_info)
            send_telegram(msg)
