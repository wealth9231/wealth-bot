...
def load_config():
    try:
        ...
        return {"dry_run": True, "max_risk_per_trade": 1.5, "max_daily_trades": 5, "allow_short": True, "trading_enabled": True}
...
def check_telegram_commands():
    ...
    try:
        ...
        if text.startswith('/'):
            handle_command(text, update.get('update_id', 0))
    except: pass
...
def handle_command(text, update_id):
    ...
    if cmd == '/stop': TRADING_ENABLED = False; response = "🛑 交易已暂停"
        ...
        try: MAX_RISK_PER_TRADE = max(0.1, min(float(cmd.split()[-1]), 2.0)); response = f"⚙️ 亏损上限调至 {MAX_RISK_PER_TRADE}U"
        except: response = "❌ 格式错误"
...
def close_all_positions():
    ...
    if DRY_RUN: simulated_positions = {}; send_telegram("🧪 [模拟] 一键清仓已触发"); return
    positions = fetch_real_positions()
    for p in positions:
...
def compute_adx(highs, lows, closes, period=14):
    ...
    plus_di = sum(plus_dm_list[:period]) / period; minus_di = sum(minus_dm_list[:period]) / period
...
def adaptive_parameters(adx, atr, price, market_state):
    ...
    if market_state == "趋势市":
        if adx > 40: stop_mult, tp_mult, position_pct, rsi_buy_max, rsi_sell_min, min_rr = 2.5, 3.5, TARGET_PROFIT_PCT * 1.3, 75, 25, 1.1
        elif adx > 30: stop_mult, tp_mult, position_pct, rsi_buy_max, rsi_sell_min, min_rr = 2.0, 3.0, TARGET_PROFIT_PCT, 70, 30, 1.2
        else: stop_mult, tp_mult, position_pct, rsi_buy_max, rsi_sell_min, min_rr = 1.5, 2.5, TARGET_PROFIT_PCT * 0.7, 65, 35, 1.3
    ...
    return {'stop_mult': stop_mult, 'tp_mult': tp_mult, 'position_pct': position_pct, 'rsi_buy_max': rsi_buy_max, 'rsi_sell_min': rsi_sell_min, 'min_rr': min_rr}
...
def place_order(symbol, side, qty, leverage, stop_loss, take_profit):
    ...
    if DRY_RUN:
        ...
        simulated_positions[symbol] = {'symbol': symbol, 'direction': side, 'entry_price': entry_price, 'qty': int(qty), 'pnl': 0.0, 'stop_loss': stop_loss, 'take_profit': take_profit}
...
def format_brief(coins_data):
    ...
    mode_text = "🧪 模拟" if DRY_RUN else "💰 实盘"
    if TRADING_ENABLED: lines.append(f"│ 🟢 {mode_text}中  /status 查详情")
    else: lines.append(f"│ 🔴 已暂停  /start 恢复")
    lines.append("└─────────────────────")
    lines.append("")
...
def run_strategy():
    ...
    for c in coins:
        ...
        if adx > 20: market_state, strategy = "趋势市", "趋势跟踪"
        ...
        if market_state == "趋势市":
            if ema12 > ema26 and price > ema12 and rsi < adaptive['rsi_buy_max']:
                direction = "buy"; stop_loss = price - adaptive['stop_mult'] * atr; take_profit = price + adaptive['tp_mult'] * atr
            elif ema12 < ema26 and price < ema12 and rsi > adaptive['rsi_sell_min'] and ALLOW_SHORT:
                direction = "sell"; stop_loss = price + adaptive['stop_mult'] * atr; take_profit = price - adaptive['tp_mult'] * atr
            else: continue
        elif market_state == "震荡市":
            if price <= bb_lower * 1.02 and rsi < adaptive['rsi_buy_max']:
                direction = "buy"; stop_loss = price - adaptive['stop_mult'] * atr; take_profit = price * 1.005
            elif price >= bb_upper * 0.98 and rsi > adaptive['rsi_sell_min'] and ALLOW_SHORT:
                direction = "sell"; stop_loss = price + adaptive['stop_mult'] * atr; take_profit = price * 0.995
            else: continue
        else: continue
