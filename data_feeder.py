...
def check_telegram_commands():
    ...
    try:
        ...
        if msg.get('text', '').startswith('/'):
            handle_command(msg['text'], update.get('update_id', 0))
    except:
        pass
...
def place_order(symbol, side, qty, leverage, stop_loss, take_profit):
    ...
    try:
        ...
        if stop_loss > 0:
            exchange.create_order(symbol, 'stop', 'sell' if side == 'buy' else 'buy', order_size, stop_loss, {'stopPrice': stop_loss, 'reduce_only': True})
        if take_profit > 0:
            exchange.create_order(symbol, 'limit', 'sell' if side == 'buy' else 'buy', order_size, take_profit, {'reduce_only': True})
        return order_size
    except:
        return 0
...
def format_signal_card(symbol, direction, market_state, price, adx, rsi, stop_loss, take_profit, qty, strategy, adaptive_info):
    ...
    price_str = f"{current_price:.6f} ({price_change:+.2f}%)"
    ...
    total_pnl_str = f"+{total_pnl:.4f}U" if total_pnl >= 0 else f"{total_pnl:.4f}U"
